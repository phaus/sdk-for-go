...
// image.
func (srv *Storage) GetFilePreview(FileId string, Width int, Height int, Gravity string, Quality int, BorderWidth int, BorderColor string, BorderRadius int, Opacity float64, Rotation int, Background string, Output string) (map[string]interface{}, error) {
	r := strings.NewReplacer("{fileId}", FileId)
...
// attachment' header.
func (srv *Storage) GetFileView(FileId string) (map[string]interface{}, error) {
	r := strings.NewReplacer("{fileId}", FileId)
